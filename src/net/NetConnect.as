    ...
    private function netStatus(event:NetStatusEvent):void{
        switch(event.info.code){
            ...
            case "NetGroup.Connect.Success":
                connected = true;
                GameApp.instance.stateMachine.dispatchEvent("complete");
                break;
    ...
    private function receiveMessage(message:Object):void{
        if(connected) {
            switch (message.data.action) {
                case "passTurn":
                    GameApp.instance.stateMachine.dispatchEvent("passed");
                    break;
                case "lines":
                    for each(var _units:Vector.<UnitView> in GameApp.instance.units){
                        for each(var unit:UnitView in _units) {
                            if (unit.owner.id == message.data.id) {
                                GameApp.instance.drawFromBuildingLine(unit, message.data.x, message.data.y);
                                break;
                            }
                        }
                    }
                ...
                case "place":
                    ...
                    var player:Player = GameApp.instance.players[message.data.team];
                    GameApp.instance.createBuilding(message.data.x, message.data.y, message.data.type, player);
                    break;
            }
        }
    }
