    ...
    public function runUnit(unit:Unit):void{
        if(!cooldownTimer || !cooldownTimer.running) {
            if(damage){
                ...
                if(owner.aoe){
                    for each(var unitView:UnitView in GameApp.instance.units[unit.player.team]) {
                ...
                if(owner.squadronPoints) {
                    if (owner.maxUnits == 0 || owner.unitsCreated < owner.maxUnits) {
                        for (var i:int = 0; i < owner.squadronPoints.length; i++) {
                            var unit:UnitView = GameApp.instance.createBuilding(owner.x, owner.y, unitCreated, owner.player, owner);
                            unit.owner.squadronPos = i;
                        }
                    }
                } else {
                    GameApp.instance.createBuilding(owner.x, owner.y, unitCreated, owner.player, owner);
                }
                owner.unitsCreated++;
            }
            if(cooldownTimer){
